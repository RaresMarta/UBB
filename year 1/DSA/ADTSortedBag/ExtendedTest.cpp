...
void testQuantity(Relation r) {
    ...
    int count = 6000;
    assert(sb.get_size() == 6000);
    for (int i = 0; i < 5000; i++) {
        TComp nr = rand() % 8000 - 4000;
        if (sb.remove(nr) == true) {
            count--;
        }
        assert(sb.get_size() == count);
    }
    assert(sb.get_size() == count);
}
